...
class Deal(CliMixin):
    ...
    def open(self, ask_id: int, bid_id: int) -> dict:
        command_args = ['deal', 'open', str(ask_id), str(bid_id), '--out', 'json']
        return self._call_command(command_args)

    def status(self, deal_id: int) -> dict:
        command_args = ['deal', 'status', str(deal_id), '--out', 'json']
        return self._call_command(command_args)

    def close(self, deal_id: int) -> dict:
        command_args = ['deal', 'close', str(deal_id), '--out', 'json']
        return self._call_command(command_args)
